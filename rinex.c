    ...
    char tobs[MAXOBSTYPE][4], sigind_t* ind)
{
    ...
    for (i = n = 0; *tobs[i]; i++, n++) {
        ind->code[i] = obs2code(tobs[i] + 1);//ind->code存的是观测值类型（1C/1Y）在obscode数组对应的索引
        ind->type[i] = (p = strchr(obscodes, tobs[i][0])) ? (int)(p - obscodes) : 0;
        ind->idx[i] = code2idx(sys, ind->code[i]);//观测值类型对应的频率
    ...
    nav_t* nav)
{
    ...
    /* read RINEX file */
    stat = readrnxfp(fp, ts, te, tint, opt, flag, index, type, nav);
    ...
    double tint, const char* opt, nav_t* nav)
{
    ...
    if (!*file) {
        return readrnxfp(stdin, ts, te, tint, opt, 0, 1, &type, nav);
    }
    ...
    for (i = 0; i < n && stat >= 0; i++) {
        stat = readrnxfile(files[i], ts, te, tint, opt, 0, rcv, &type, nav);
    }
    ...
    nav_t* nav)
{
    ...
    return readrnxt(file, rcv, t, t, 0.0, opt, nav);
}
...
extern int readrnxc(const char* file, nav_t* nav)
{
    ...
    /* read rinex clock files */
    for (i = 0; i < n; i++) {
        if (readrnxfile(files[i], t, t, 0.0, "", 1, index++, &type, nav)) {
            continue;
        }
        stat = 0;
        break;
...
extern int open_rnxctr(rnxctr_t* rnx, FILE* fp)
{
    ...
    if (!readrnxh(fp, &ver, &type, &sys, &tsys, tobs, &rnx->nav)) {
