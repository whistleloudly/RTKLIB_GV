...
#define FREQ1       1.57542E9           /* L1/E1/B1C  frequency (Hz)1.57542E9 */
...
#define DDMODE_STANDARD     0           /*DD mode standard;hy add 20210315*/
#define SDMODE_STANDARD     1           /*SD mode;hy add 20210315*/
...
/* type definitions ----------------------------------------------------------*/
    ...
    typedef struct {        /* solution type */
        ...
        double Lbias[5];       //hy add 20210319:输出Lbias值
        double Lbias_var[5];   //hy add 20210621:输出相位LBias var
        double pf_begin;
        double pf_consuming;/*hy add 20210325: add for recording time consuming by each epoch*/
        double pf_only_begin;
        double pf_only_consuming;   /*hy add 20210917: add for recording time consuming for PF method only*/
        double CLbias[5];
        double QCLbias[5];
        int NPARTICLE;
        double dop[4];      /*hy add 20211012: add for output dop value*/
    } sol_t;
    ...
    typedef struct {        /* processing options type */
        ...
        int ddmode;         /*hy add 20210315:DD/SD*/
        int pfnum;          /*hy add 20210317:the number particle filter,0 mean no PF(0:no particle filter,default 200)*/
        int pfoutput;       /*hy add 20210325:for recording the Lbia&ratio&ENU of the first epoch(0:off,1:on)*/
        int est_CLbias;     /*hy add 20210405:for est Lbias of pseudorange(0:off,1:substitute ww PLbias,2:est_Code_Lbias(continuous),3:est_Code_Lbias(instantaneous),4:fix_est_Code_Lbias)*/
        int pfopt;          /*hy add 20210728:for setting particle filter optimization parameters(0:no optimization,1:improved particle filter,2:adaptive particle number,3:improved particle filter+adaptive particle number)*/
    ...
    typedef struct {        /* solution options type */
        ...
        /* hy add 20210929: output file flags */
        int out_cslip;          /* cycle slip log flag(0:off,1:on) */
        int out_LB;             /* phase LBias log flag(0:off,1:on) */
        int out_res;            /* resp and resc log flag(0:off,1:on) */

        /* wmk20140612 processing session */
        gtime_t start_time; /* processing start time */
        gtime_t end_time;   /* processing end time */
        double  interval;   /* processing interval */
    } solopt_t;
    ...
    typedef struct {        /* satellite status type */
        ...
        double slipthres[NFREQ];  /* hy add 20210929: the thres of cycle-slip*/
        double slipvalue[NFREQ];  /* hy add 20210929: the value of cycle-slip*/
    ...
    typedef struct {        /* RTK control/result type */
        ...
        int NPARTICLE;      /*hy add 20210317: the number of particle*/
        double* Lbias;      /*all Lbias particle*/
        double* Lratio;
        double* Pratio;
        double Lbias_particle;
        double* bias_e;     /*hy add 20210324: add for output temp enu for analysis*/
        int fp_Lbias_flag;
    } rtk_t;
    ...
    EXPORT int readrnxt(const char* file, int rcv, gtime_t ts, gtime_t te,
        double tint, const char* opt, nav_t* nav);
    ...
    EXPORT void satantoff(gtime_t time, const double* rs, int sat, const nav_t* nav,
        double* dant);
    EXPORT int  satpos(gtime_t teph, int sat, int ephopt,
        const nav_t* nav, double* rs, double* dts, double* var,
        int* svh);
    EXPORT void satposs(gtime_t time, int n, const nav_t* nav,
    ...
    /*-------------------------------added by wmk20140612---------------------------------------------*/
    extern void  rtkopenoutput(char* file, const solopt_t* solopt, const prcopt_t* prcopt, char* name);
    extern void  rtkopenoutput_cslip(char* file, const solopt_t* solopt, const prcopt_t* prcopt, char* name);
    extern void rtkcloseoutput(void);
