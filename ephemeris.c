...
static int ephpos(gtime_t teph, int sat, const nav_t* nav,
    int iode, double* rs, double* dts, double* var, int* svh)
{
    ...
    *svh = -1;
    ...
    if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP || sys == SYS_IRN) {
        ...
        eph2pos(teph, eph, rs, dts, var);
        teph = timeadd(teph, tt);
        eph2pos(teph, eph, rst, dtst, var);
        *svh = eph->svh;
    }
    else if (sys == SYS_GLO) {
        if (!(geph = selgeph(teph, sat, iode, nav))) return 0;
        geph2pos(teph, geph, rs, dts, var);
        teph = timeadd(teph, tt);
        geph2pos(teph, geph, rst, dtst, var);
        *svh = geph->svh;
    }
    else if (sys == SYS_SBS) {
        if (!(seph = selseph(teph, sat, nav))) return 0;
        seph2pos(teph, seph, rs, dts, var);
        teph = timeadd(teph, tt);
        seph2pos(teph, seph, rst, dtst, var);
        *svh = seph->svh;
    }
    else return 0;
    ...
    double* rs, double* dts, double* var, int* svh)
{
    ...
    if (i >= nav->sbssat.nsat) {
        ...
        ephpos(teph, sat, nav, -1, rs, dts, var, svh);
        *svh = -1;
        return 0;
    }
    /* satellite postion and clock by broadcast ephemeris */
    if (!ephpos(teph, sat, nav, sbs->lcorr.iode, rs, dts, var, svh)) return 0;
    ...
    int opt, double* rs, double* dts, double* var, int* svh)
{
    ...
    /* satellite postion and clock by broadcast ephemeris */
    if (!ephpos(teph, sat, nav, ssr->iode, rs, dts, var, svh)) return 0;
...
*-----------------------------------------------------------------------------*/
extern int satpos(gtime_t teph, int sat, int ephopt,
    ...
    int* svh)
{
    ...
    *svh = 0;
    ...
    switch (ephopt) {
    case EPHOPT_BRDC: return ephpos(teph, sat, nav, -1, rs, dts, var, svh);
    }
    *svh = -1;
    return 0;
}
...
*-----------------------------------------------------------------------------*/
extern void satposs(gtime_t teph, int n, const nav_t* nav,
    int ephopt, double* rs, double* dts, double* var, int* svh)
{
    ...
    for (i = 0; i < MAXSAT; i++) {
        ...
        if (!satpos(teph, sat, ephopt, nav, rs + i * 6, dts + i * 2, var + i,
            svh + i)) {
        ...
        if (dts[i * 2] == 0.0) {
            if (!ephclk(time[i], teph, sat, nav, dts + i * 2)) continue;
            dts[1 + i * 2] = 0.0;
            *var = SQR(STD_BRDCCLK);
        }
    }
